...
contract Spender {
    ...
    error Spender_ReceiveTransferFailedWithWrongAddress();
    ...
    // must verify if it's true
    function receiveTransfer(uint256 value) external {
        if (msg.sender != s_manualToken) revert Spender_ReceiveTransferFailedWithWrongAddress();

        s_balanceAmount += value;
    }

    function receiveApproval(address _from, uint256 _value, address _token, bytes calldata /* _extraData */ )
        external
    {
